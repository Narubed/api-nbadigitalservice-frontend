...
export default function UserMoreMenu({ value }) {
  ...
  const deleteProvince = async (e) => {
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: 'คุณต้องการลบอำเภอนี้ออกหรือไม่ !',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        Swal.fire({
          icon: 'success',
          title: 'คุณได้ลบอำเภอนี้ออกเเล้ว',
          showConfirmButton: false,
          timer: 1500,
        });
        await axios.delete(`${process.env.REACT_APP_WEB_GEO_BACKEND}/amphure/${value.amphur_id}`, {
          data,
        });
        setTimeout(() => {
