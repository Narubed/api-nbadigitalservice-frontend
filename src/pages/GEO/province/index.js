...
import { ProvinceListHead, ProvinceToolbar, ProvinceMoreMenu } from '../../../sections/@dashboard/geo/province';
// mock
...
export default function User() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = province.map((n) => n.province_name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - province.length) : 0;

  const filteredUsers = applySortFilter(province, getComparator(order, orderBy), filterName);
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button
            ...
            to="/dashboard/geo/create-province"
        ...
        <Card>
          <ProvinceToolbar numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { province_id, province_name, nba_geo_name, nba_zone } = row;
